...
public class Stream {
	...
	final boolean compareCrcs() {
		...
		Utils.CRC32Instance.update(payload, 0, offset);
		int crc = (int) Utils.CRC32Instance.getValue();
		int realCRC = getInt();
		return realCRC == crc;
	}
	...
	final String getJStr() {
		...
		if (k == 0) {
			...
			return Utils.getStringFromBytes(payload, j, k);
		}
	}
	...
	final int putCrc(int off) {
		Utils.CRC32Instance.reset();
		Utils.CRC32Instance.update(payload, off, offset);
		int crc = (int) Utils.CRC32Instance.getValue();
		putInt(crc);
		return crc;
	}
	...
	final void putJStr(String s) {
		...
		offset += Utils.getStringBytes(s, 0, s.length(), payload, offset);
		payload[offset++] = 0;
	}
